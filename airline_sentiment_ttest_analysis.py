...
def perform_ttest_analysis(df, airline1, airline2):
    """Perform t-test analysis for all issue categories"""
    print(f"\n{'='*60}")
...
def main():
    ...
    try:
        ...
        if not available_airlines:
            print("No airlines found in database. Please ensure data is loaded.")
            return
        
        # Select comparison airline
        comparison_airline = select_comparison_airline(available_airlines)
        
        if not comparison_airline:
            return
        
        airline1 = 'AmericanAir'
